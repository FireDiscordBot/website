					...
					{!isSearching ? (
						<div className="row">
							<div className="col-md-3">
								...
								<p className={"category-note"}>{props.categories[currentCategory].Note}</p>
								...
								<div id="categories">
									<ul id="categories-list" className="list-unstyled">
										{props.categories.map((e) => (
											<li
												key={e.id}
												className={e.id === currentCategory ? "c-active noselect" : "noselect"}
								...
								<div className="row">
									...
									<table className="table">
										...
										<tbody className="table-body">
											{Object.entries(props.categories[currentCategory].commands)
									...
									<div className="row">
										...
										<div className="table-responsive" style={{ paddingBottom: "0px" }}>
											<table className="table table-striped">
												...
												<tbody className="table-body">
													{Object.entries(props.categories[currentCategory].flags)
														.map(([name, obj]) => [name, ...Object.values(obj)])
														.map((commandFields) => (
															<tr key={commandFields}>
																<td key={commandFields[1]} className={"name"}>
...
Commands.getInitialProps = async function () {
	const res = await fetch("https://aether.gaminggeek.dev/commands", {
		headers: { "User-Agent": "Fire Website" },
		mode: 'no-cors'
	});
	const res2 = await fetch("https://aether.gaminggeek.dev/allcommands", {
		headers: { "User-Agent": "Fire Website" },
		mode: 'no-cors'
	});
