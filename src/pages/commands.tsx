...
const Commands: NextPage<Props> = ({categories, commands}) => {
  ...
  const prefix = router.query.prefix && router.query.prefix.length <= 10 ? router.query.prefix as string : "$";
  ...
  if (!isSearching) {
    const categoriesComponents = categories.map((category) => {
      ...
      return (
        <li
          key={category.id}
          className={category.id === currentCategoryId ? "c-active no-select" : "no-select"}
          onClick={handleClick}
        >
          {category.name}
        </li>
...
Commands.getInitialProps = async () => {
  ...
  const [categories, commands] = (await Promise.all(promises.map(response => response.json()))) as [Category[], Command[]];
